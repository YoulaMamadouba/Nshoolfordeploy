    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {tenants.map((tenant, index) => (
        <motion.div
          ...
          animate="visible"
          whileHover="hover"
          className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl border border-gray-100 overflow-hidden transition-all duration-300 min-h-[280px]"
        >
          ...
          <div className="relative p-6 pb-4">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-gray-900 text-lg leading-tight mb-1 truncate">{tenant.name}</h3>
                  <p className="text-sm text-gray-500 font-mono truncate">{tenant.code}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <motion.span
                ...
                whileHover="hover"
                className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold ${getStatusColor(tenant.status).bg} ${getStatusColor(tenant.status).text} ${getStatusColor(tenant.status).border} shadow-sm`}
              >
              ...
              <motion.span
                variants={badgeVariants}
                whileHover="hover"
                className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold ${getPlanColor(tenant.plan).bg} ${getPlanColor(tenant.plan).text} ${getPlanColor(tenant.plan).border} shadow-sm`}
              >
